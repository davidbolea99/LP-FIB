...
instance Show Board where
    show board@(Board _ width _ _) = header ++ border_up ++ separated_board ++ border_down
        where
            ...
            rowSeparator = "+" ++ (intercalate "+" $ take width $ repeat "---") ++ "+\n   "
...
loop board ai ((X,move):moves) = do
    ...
    let end = check_win newBoard
    
    if end then do return True
    else        do loop newBoard ai moves
